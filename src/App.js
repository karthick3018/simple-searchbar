...
const App=(props)=> {
  ...
  const filterName = debounce((value) => {
    const finalNameResults = searchNamesInList(value);
    setNameSuggestionList(finalNameResults);
  }, 400);


  const searchNamesInList = (value)=>{
    if (value) {
      const filteredData = filter(unModifiedNameList, eachResult => eachResult.toLowerCase().includes(value.toLowerCase()));
      return filteredData;
    }
    return [];
  }
     ...
     />
      ...
      {nameSuggestionList && 
       nameSuggestionList.map((eachName,index)=>{
            return(
              index<20 && <li onClick={()=>handleOnClick(eachName)} key={ index }>{ eachName }</li>
            )
          })}
   </div>
  );
